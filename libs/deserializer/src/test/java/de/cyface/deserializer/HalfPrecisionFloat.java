...
public class HalfPrecisionFloat {
    ...
    public HalfPrecisionFloat(final float number) {
        ...
        if (number < Short.MIN_VALUE) {
            final int val = fromFullPrecision(Short.MIN_VALUE);
            this.halfPrecision = (short)val;
            return;

            // FIXME: reactivate and delete the above but we need a handling for values outside this range [STAD-308]
        ...
        /*
         * Kommentar: Wenn das aktiviert ist, dann wird der original Wert für den float
         * auf den parameter geschrieben und beim direkten auslesen über getFullFloat()/getFullDouble() auch nur dieser
         * Originalwert wieder zurückgegeben. Diese Methoden sollen aber die aus dem 16Bit Short rekonstruierten
         * float/double Werte zurückgeben. Ansonsten könnte man auch keinen Genauigkeitsvergleich machen, da in dem Fall
         * immer der original 32-Bit float zurückegeben wird.
         */
    }
